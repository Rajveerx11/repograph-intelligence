...
#### 11.15 VS Code Extension

The extension under `apps/vscode/` brings RepoGraph into the editor inner loop. It spawns the canonical MCP server as a child process and speaks JSON-RPC over stdio — no embedded analyzer, no telemetry, no cloud. Features: a blast-radius CodeLens above the first top-level declaration of every JS / TS / Python file (`blast radius N (M direct) · risk LEVEL`), a Risk Ranking sidebar in the activity bar that surfaces critical-blast-zone files with reasons, a status-bar refresh trigger, and palette commands for `Analyze Workspace`, `Show Blast Radius for Active File`, `Run Architecture Policy`, and `Check Drift Against Baseline`. The MCP client caps its inbound buffer at 8 MB and bounds every request at 60 s; the extension refuses to spawn the subprocess in an untrusted workspace so a hostile `.vscode/settings.json` cannot turn `mcpCommand` into an arbitrary-code-execution vector. Config-supplied `policyPath` and `baselinePath` flow through a `path.relative` containment check; MCP-returned row paths are validated the same way before the Risk view wires up a `vscode.open` click handler.

#### 11.16 GitHub Action (PR bot)

The composite action under `apps/github-action/` is RepoGraph's pull-request distribution channel. One workflow step installs the CLI, analyzes the PR head once, runs every enabled gate (`policy`, `api-diff`, `drift`, `test-select`), and posts a single sticky verdict comment on the PR. The comment uses an HTML marker so subsequent runs update the existing comment in place; teams who want a per-run audit log can flip `comment-mode: append`. Outputs `verdict` (`pass` / `fail`) and `comment-url`; exit codes mirror the CLI contract (`2` policy, `3` api-diff, `4` drift) so downstream workflow steps can branch on which gate fired. The action carries a custom zero-dependency REST client (Node's built-in `fetch`) that refuses to follow redirects and refuses to follow `Link` headers that point outside `api.github.com`, so a hostile API response cannot leak the workflow token. Every backtick-wrapped field in the Markdown comment is run through an `escapeInline` helper, and the CLI-output recovery is a brace-balanced, string-aware scan so a `}` character inside a violation message no longer truncates the report.

### Deliverables

The system should analyze repositories, generate dependency graphs, expose repository metrics, visualize repository structure interactively, export the graph as Mermaid or GraphViz DOT for embedding in documentation and pull requests, enforce architectural invariants via policy-as-code, gate releases on structural drift and public-API stability, select the minimum test set that exercises a diff, ship as a multi-arch Docker image plus a standalone Node tarball on every tag release, surface inline in VS Code as a first-class editor experience, and post a sticky verdict comment on every pull request via a maintained GitHub Action.
