...
- Ships a GitHub Action under [`apps/github-action/`](apps/github-action/) that runs every PR gate (policy, api-diff, drift) plus the informational test-selection report and posts a sticky verdict comment on every pull request; reuses the CLI's distinct exit codes (`2` / `3` / `4`) so downstream steps can branch on which gate fired and refuses to follow non-`api.github.com` redirects so the workflow token cannot leak
